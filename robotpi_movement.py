...
class Movement:
    ...
    def left_ward(self,speed=10, direction = 0, w = 0,times=100):
        if self.isOpen:
            command = self.cmd.Command(0, 5, 50, 500)
            self.action.write_serial(command)
            return True
        return False
    ...
    def move_free(self,speed=10, direction = 0, w = 0,times=100):
        if self.isOpen:
            command = self.cmd.Command(direction, speed, w,times)
            self.action.write_serial(command)
            return True
        return False
